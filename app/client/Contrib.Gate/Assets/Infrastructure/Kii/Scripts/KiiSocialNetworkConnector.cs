    ...
    public class KiiSocialNetworkConnector : MonoBehaviour
    {
                ...
                KiiSocialCallback callback)
        {
            ...
            c.CallStatic("showDialog", this.gameObject.name,
                         GetStartPointUrl(), GetEndPointUrl(), ua);
#elif UNITY_IPHONE
...
#pragma warning restore 0618
        ...
        private string GetUA(Provider provide)
        {
            switch (provider) {
                case Provider.GOOGLEPLUS:
                case Provider.GOOGLE:
                    return ConstantValues.UA_PRODUCT + "/" +
                        ConstantValues.SDK_VERSION;
                default:
                    return null;
            }
        }
